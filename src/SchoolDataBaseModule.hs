...
module SchoolDataBaseModule where
...
executeQuery :: MySQLConn -> String -> IO Bool
...
insertTable :: (TableHolder a) => MySQLConn -> a -> [String] -> IO Bool
insertTable conn table values = do
      ...
      result <- (executeQuery conn inser_query_str)
      return result
...
updateTable :: (TableHolder a) => MySQLConn -> a -> [String] -> String -> IO Bool
...
deleteTable :: (TableHolder a) => MySQLConn -> a -> String -> IO Bool
deleteTable conn table id = do
      let query_str = "DELETE FROM " ++ (name (getData table)) ++ " WHERE (id = " ++ id ++ ")"
      result <- (executeQuery conn query_str)
      return result

deleteRow :: (TableHolder a) => MySQLConn -> a -> IO ()
deleteRow conn table = do
      putStrLn "Enter id"
      id <- getLine
      result <- (deleteTable conn table id)
      if result
          then putStrLn "Delete Success"
          else putStrLn "Delete Fail"
...
updateRow conn t = do
      ...
      query_status <- updateTable conn t values id
      if query_status
            then do
                  ...
                  if check_values
                        then do 
                            ...
                            putStrLn "Update succesful"
                        else do 
                            executeQuery conn "rollback"
                            putStrLn "Update fail due to logic"
            else putStrLn "Fail update: sql error"
...
createRow conn t = do
      ...
      let values = (prepareValuesToQuery t read_values)
      executeQuery conn "BEGIN"
      query_status <- insertTable conn t values
      if query_status
            then do
                  ...
                  if check_values
                        then do 
                            executeQuery conn "commit"
                            putStrLn "Insert succesful"
                        else do 
                            executeQuery conn "rollback"
                            putStrLn "Insert fail due to logic"
            else putStrLn "Fail insert: sql error"
